...
def test(config, model, device):
    ...
    data_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=16,
                                                  collate_fn=batcher(device), shuffle=False, drop_last=False,
                                                  num_workers=0)
    ...
    for batch in tqdm(data_loader):
        opcode_batch = [tensor for tensor in batch[:7]]
...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset', type=str, default='MMAN', help='name of dataset.java, python')
