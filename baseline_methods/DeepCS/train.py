...
def train(args):
    ...
    val_set = eval(config['dataset_name'])(data_path,
                                           config['val_name'], config['name_len'],
                                           config['val_tokens'], config['tokens_len'],
                                           config['val_api'], config['api_len'],
    ...
    for epoch in range(int(args.reload_from) + 1, config['nb_epoch'] + 1):
        ...
        if epoch % args.val_every == 0:
            ...
            for batch_val in val_data_loader:
                code_batch = [tensor.to(device) for tensor in batch_val[:6]]
                doc_batch = [tensor.to(device) for tensor in batch_val[6:]]
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='DeepCS', help='name of dataset.java, python')
